...
export default function ApprovePage() {
  const { approveCandidateLoading, postApproveCandidate } =
  useSaveApprove();
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await postApproveCandidate(form);

      toast.success("Candidato aprovado no processo!");
    } catch (error: any) {
      if (error.response) {
  ...
  return (
    <Style.Container>
      <Style.Title>Aprovar Candidato</Style.Title>
      <Style.FormContainer>
        <Style.Form onSubmit={submit}>
          <InputForm
            ...
            disabled={approveCandidateLoading}
            autoComplete="off"
            required
          />
          <ButtonForm type="submit" disabled={approveCandidateLoading}>
            {approveCandidateLoading ? loadingButton : "Agendar"}
          </ButtonForm>
        </Style.Form>
